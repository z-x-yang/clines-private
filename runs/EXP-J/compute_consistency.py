...
RUN_ROOT = os.path.dirname(os.path.abspath(__file__))  # runs/EXP-J
N_RUNS = 5
...
def note_id_from_filename(fname, slice_name, run):
    """EXP-J_run{R}_{slice}_{noteid}_default.csv -> noteid."""
    base = os.path.basename(fname)
    prefix = f"EXP-J_run{run}_{slice_name}_"
    suffix = "_default.csv"
...
def load_run_note(slice_name, run, note_id):
    fname = os.path.join(RUN_ROOT, f"run{run}", slice_name,
                         f"EXP-J_run{run}_{slice_name}_{note_id}_default.csv")
    ...
    for sl in SLICES:
        ...
        for r in range(1, N_RUNS + 1):
            for f in glob.glob(os.path.join(RUN_ROOT, f"run{r}", sl,
                                            f"EXP-J_run{r}_{sl}_*_default.csv")):
