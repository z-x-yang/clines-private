...
| EXP-J2 | R5.4.5 | Run-to-run output consistency 重跑(优化代码) | P1 | **PASS** | array 41743916 (15/15 COMPLETED, 31:02–1:17:37/cell, commit `c90877e` = `a31f18a` + cache-symlink job-script fix) | mention-set Jaccard $0.60$ / code-set Jaccard $0.62$;per-mention 字段 modal agreement **$0.899$** (unit $0.94$, end_date $0.94$, begin_date $0.91$, assertion $0.85$, value $0.85$;$n=43{,}670$ cells over 20 notes × 5 runs)。set-membership 有 stochastic 抖动,per-field 内容高度稳定 → majority-vote over $R\geq 3$ runs 即可恢复 consensus | experiments/EXP-J2_output_consistency.md |
