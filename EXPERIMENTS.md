...
| EXP-G | E7 | Ablation: SapBERT / SemChunk / Date module / Step 4 | P1 | **RUNNING-eval** | Plan C/D 15 cells (2 notes/ds) 全 COMPLETED;eval harness 缺 position-build 步骤(canonical `process_entity_index.py --use_sequential`)已修,eval 进行中 | 初步信号:SapBERT off → 4CE code F1 0.75→0.25(归一化层贡献最大),mention 基本不动;完整 ΔF1 待回填 | experiments/EXP-G_ablation.md |
| EXP-G2 | E7' | Ablation grid 复跑(gpt-4o-mini-0718,EXP-G 的 sister) | P1 | **RUNNING-eval** | 15 cells (2 notes/ds) 全 COMPLETED;与 EXP-G 同款 eval 进行中 | 测 per-module 贡献是否在更便宜模型上 generalize(R5 cost / model-agnostic 论点);ΔF1 待回填 | experiments/EXP-G2_ablation_gpt4omini.md |
