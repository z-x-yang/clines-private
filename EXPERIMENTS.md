...
| EXP-F | E6 | Baseline 补强：o3-mini single-prompt + GPT-4o CoT single-call | P0 | PASS | **新跑**（小规模） | code F1 比 CLINES 4-step 低 +0.26~+0.38（最保守主结果候选），reasoning model 也补不上且成本最高 → 支撑 R5 A1 架构论点（主结果参照系待确认） | experiments/EXP-F_baseline_extra.md |
...
- 2026-05-26：**EXP-F 跑完 → PASS**。6 个 (model,dataset) job COMPLETED，eval 完成（含修复 `run_eval.sh` 自引用 symlink bug 后重跑的 4CE）。两个 single-call baseline 在 code F1 上一致低于 CLINES 4-step（+0.26~+0.38，对最保守主结果候选 150537），且 o3-mini reasoning 也补不上、成本最高（\$9.63，4× completion tokens）→ 支撑 R5 A1。Open item：`outputs/evaluation_results_0904/` 下 3 个 gpt4_eval 时间戳数值分歧，canonical 主结果参照待 user 确认（影响 rebuttal 报的确切 ΔF1，不影响 PASS 方向）。详见 EXP-F_baseline_extra.md §8-11。
