...
| EXP-E2 | E5 | Hallucination 7-class FP taxonomy（split val/date + broader not_an_error） | P0 | INCONCLUSIVE-pending-review | 700 GPT-4.1 judge calls（0 失败，mean conf 0.97） | 6/7 类 < 20%：wrong_code 16.7% / wrong_assertion 8.7% / wrong_value 8.3% / fabricated 5.4% / boundary 5.4% / wrong_date 1.2%；`not_an_error` 54.3% 超阈值（部分合法 annotation gap，部分是判 "pred 推断日期可接受" 的 lenient 解读，需 user 决策 Strict vs Lenient）；true hallucination = 45.7% of FP = 24.4% of 13,454 preds；60-row user review sample 待 user 填 | experiments/EXP-E2_judge_6class.md |
