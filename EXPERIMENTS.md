...
| EXP-F | E6 | Baseline 补强：o3-mini single-prompt + GPT-4o CoT single-call | P0 | PASS | **新跑**（小规模） | 对 canonical 主结果 160641，CLINES 4-step 在所有可比列都赢，code F1 高 +0.37~+0.54；reasoning model 补不上且成本最高 → 支撑 R5 A1 架构论点 | experiments/EXP-F_baseline_extra.md |
...
- 2026-05-26：**EXP-F 跑完 → PASS**。6 个 (model,dataset) job COMPLETED，eval 完成（含修复 `run_eval.sh` 自引用 symlink bug 后重跑的 4CE）。**主结果参照系经 paper Figure 3 绘图链反推锁定 = `gpt4_eval_20250904_160641`**（`plot_figure3.py`→`runs/EXP-BC/metrics_with_ci.json` 与之精确吻合，且与 baseline 同款 position-overlap 匹配 / 同 gold 域；150537/154356 是不同匹配域，不可比）。对 160641：两个 single-call baseline 在**所有可比列都低于** CLINES 4-step，**code F1 低 +0.37~+0.54**，value/unit 也全输；o3-mini reasoning 补不上且成本最高（\$9.63，4× completion tokens）→ 支撑 R5 A1。残留低风险：2025 原始投稿图是否同用 160641 未 100% 核实（rebuttal 随 revision 出，内部一致用 160641）。详见 EXP-F_baseline_extra.md §8-11。
