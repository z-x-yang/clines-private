...
| EXP-A3 | E1 | IAA second-opinion methodology：3-layer cascade (L1 exact = EXP-A2, L2 UMLS sibling + value/unit fuzzy, L3 Claude rule-based judge) | P0 | FAIL | 0（同 EXP-A2 输入；新 UMLS dict 文件已 gitignore） | L1+L2+L3 κ_entity 0.38(4CE)/0.49(CORAL)/0.45(overall); PABAK 0.48/0.53/0.51。974 disagreements 中 0 升级 actually_agreed（honesty preserved），21% 标 ambiguous（admin tokens / 错 AI 类 / KUMC_1 outlier / breastca_38 policy），79% true_disagree。**FAIL** vs 阈值 κ>0.50 OR PABAK>0.60；**推荐主报仍用 EXP-A2 PABAK 0.46**，EXP-A3 数字作 supplement sensitivity bound（详情见 .md §11）。 | experiments/EXP-A3_enhanced_agreement.md |
...
- 2026-05-26：User 看完 EXP-A2 仍觉 IAA 数字偏低，要求加 "更好的判断方法" 但**拒绝 GPT-4.1 作 judge**（self-audit bias），允许 **Claude (different model) 作 second opinion**。新立 **EXP-A3**：3-layer cascade 在 EXP-A2 之上：L1=exact (=EXP-A2)，L2=UMLS Semantic-Group sibling + value/unit/date fuzzy（deterministic），L3=Claude rule-based judge（5 named rules + default true_disagree）。结果：overall κ 0.382→0.447 (+0.066), PABAK 0.456→0.514 (+0.058)。**0 of 974 disagreements 升级 actually_agreed**（honesty preserved，符合 user 警告的 KUMC_1 'against her will' 例外）；21% 标 ambiguous，79% 维持 true_disagree。**FAIL** vs PASS 阈值（κ>0.50 OR PABAK>0.60）—信息性 negative result。**Recommendation: 主报仍用 EXP-A2 PABAK 0.46**，EXP-A3 作 supplement sensitivity bound（Methods §3.3 添加一段说明 21% policy-driven / 79% substantive）。
