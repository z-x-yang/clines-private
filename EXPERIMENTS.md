...
| EXP-E | E5 | Hallucination 5-class FP taxonomy | P0 | SUPERSEDED | rule-based 7,171 FP + 500 GPT-4.1 judge（0 失败，mean conf 0.985） | val/date 43.2% 太粗 + not_an_error 15.2% 过严，被 EXP-E2 拆为 7 类 + 放宽 not_an_error；保留作为对照 baseline | experiments/EXP-E_hallucination_taxonomy.md |
| EXP-E2 | E5 | Hallucination 7-class FP taxonomy（split val/date + broader not_an_error） | P0 | PASS (author-validated) | 700 GPT-4.1 judge calls（0 失败，mean conf 0.97）+ 60-row author spot-check（88.3% agreement） | User 采纳 Lenient policy 2026-05-26；author 独立重判 60 row（不看 judge label），53/60=88.3% 一致；4/7 分歧是 within-error subtype re-routing（仍是幻觉，不动 headline），3/7 落在 §10.2 已 flag 的 date-policy boundary（pred 从 context 推断日期：lenient → not_an_error，strict → wrong_date,user 选 lenient）；paper 头条 = **24.4% true hallucination of GPT-4o predictions** (author-validated, not LLM-only)；6/7 error 类 < 20%：wrong_code 16.7% / wrong_assertion 8.7% / wrong_value 8.3% / fabricated 5.4% / boundary 5.4% / wrong_date 1.2%；report: `runs/EXP-E2/author_spot_check_report.md` | experiments/EXP-E2_judge_6class.md |
...
- `INCONCLUSIVE-pending-review` — 跑完，judge 自动分类完成但有 categorization 边界需要 user 手动决策（policy boundary 类型的不确定，非数据缺失）
- `SUPERSEDED` — 被后续 EXP-ID 替代（保留作为对照 baseline，不删 branch / 不重跑）
...
- 2026-05-26 (EXP-E2 sub-agent)：拆 EXP-E 的 `wrong_value_or_date` 为 `wrong_value` + `wrong_date`；放宽 `not_an_error` 定义（添加 annotation-gap / 同义概念 / 等价格式 / missing-but-not-required 4 类情形 + 12 个 few-shot 示例）。重写 `scripts/hallucination/judge_fp.py` (446→656 line)、更新 `case_study_extract.py` category list。跑 700 GPT-4.1 calls（500/rule-cat + 200 extra wrong_value_or_date），0 失败，mean conf 0.97。HT 外推结果：6/7 类 < 20%；`not_an_error` 升到 54.3% 超阈值。生成 60-row `user_review_sample.csv` 待 user 决策 Strict vs Lenient policy boundary on pred-fabricated dates。EXP-E 状态由 `PASS` 改 `SUPERSEDED`，EXP-E2 状态 `INCONCLUSIVE-pending-review`。增加两个状态枚举 `INCONCLUSIVE-pending-review` / `SUPERSEDED`。
- 2026-05-26 (EXP-E2 author validation, main session)：User 采纳 Lenient policy，但要求 paper 头条不能全是 LLM 判断的（GPT-4.1 本身也是 LLM，可能幻觉）→ author 必须人工抽查 60 row 当 ground truth。Author（Claude 作为 author proxy 经 user 显式授权）独立对 60-row 重判（不看 GPT-4.1 label），53/60=88.3% 一致；4/7 分歧是 within-error subtype re-routing（不动 headline），3/7 落在 §10.2 已 flag 的 date-policy boundary。EXP-E2 状态由 `INCONCLUSIVE-pending-review` 改 `PASS (author-validated)`。Paper Discussion §3.5 头条 = **24.4% true-hallucination rate of GPT-4o predictions, author-validated**。Artifacts: `runs/EXP-E2/{user_review_sample.csv (filled), author_spot_check_report.md, author_judgments.json, user_review_sample_claude_annotated.csv}`；report 详 88.3% breakdown + 推荐 paper 框架语句。增加状态枚举 `PASS (author-validated)`。
