...
if __name__ == '__main__':
    ...
    notes = [item for item in os.listdir(args.notes_file) if item.endswith('.txt')]
                        
    for i in tqdm(range(args.start_index, len(notes)), desc="Processing notes"):
        # ehr = notes.iloc[i]['TEXT']
        # key = str(notes.iloc[i]['ROW_ID'])
        print(os.path.join(args.notes_file, notes[i]))
        with open(os.path.join(args.notes_file, notes[i]), 'r') as f:
