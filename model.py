...
class Retriever():
    ...
    def embed_dictionary(self, batch_size=256):
        ...
        print(f"Checking if cache file exists at {
        ...
        if os.path.exists(cache_file + '/dense_embed_all.pt'):
            ...
            self.dense_embeds_all = torch.load(
                cache_file + '/dense_embed_all.pt', map_location=torch.device('cpu'))
            self.term_list_all = json.load(
                open(cache_file + '/term_list_all.jsonl', 'r'))
            self.dense_embeds_all = torch.load(
                cache_file + '/dense_embed_all.pt')
        else:
            ...
            json.dump(self.term_list_all, open(
                cache_file + '/term_list_all.jsonl', 'w'))
        ...
        if os.path.exists(cache_file + '/dense_embed_bodyloc.pt'):
            print("Cache file found. Loading dense embeddings from cache.")
            self.term_list_bodyloc = json.load(
                open(cache_file + '/term_list_bodyloc.jsonl', 'r'))
            self.dense_embeds_bodyloc = torch.load(
                cache_file + '/dense_embed_bodyloc.pt', map_location=torch.device('cpu'))
        else:
            print("Cache file not found. Embedding terms and saving to cache.")
            self.encoder.eval()
            ...
            torch.save(self.dense_embeds_bodyloc,
                       cache_file + '/dense_embed_bodyloc.pt')
            json.dump(self.term_list_bodyloc, open(
                cache_file + '/term_list_bodyloc.jsonl', 'w'))
            print("Dense embeddings saved to cache.")
